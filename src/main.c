...
int escolher_classe(){
    ...
    int c = 0; if (scanf("%d", &c) != 1) { while (getchar()!='\n'); return -1; }
    return c-1;
};
...
void adicionar_item_exemplo(Player *p){
    ...
    strncpy(med.nome, "Medkit", 31); med.nome[31] = '\0'; med.poder = 25; med.quantidade = 2;
    inventory_add(&p->inventario, med);

    Item pist;
    pist.tipo = ITEM_PISTOLA;
    strncpy(pist.nome, "Pistola", 31); pist.nome[31] = '\0'; pist.poder = 6; pist.quantidade = 1;
    inventory_add(&p->inventario, pist);

    Item muni;
    muni.tipo = ITEM_MUNI; 
    strncpy(muni.nome, "Municao", 31); muni.nome[31] = '\0'; muni.poder = 1; muni.quantidade = 12;
    ...
    while (executando)
    {
        ...
        if(op == 1){
            ...
            while (in_game)
            {
                ...
                char cmd = 0; while (getchar()!='\n'); // limpa
                cmd = getchar(); while (getchar()!='\n');

                int moved = 0;
                if (cmd == 'w') moved = map_move_player(&mapa, &jogador, 0, -1);
                else if (cmd == 's') moved = map_move_player(&mapa, &jogador, 0, 1);
                else if (cmd == 'a') moved = map_move_player(&mapa, &jogador, -1, 0);
                else if (cmd == 'd') moved = map_move_player(&mapa, &jogador, 1, 0);
                else if (cmd == 'i') { inventory_list(&jogador.inventario); pause_console(); continue; }
                else if (cmd == 'p') { player_print_stats(&jogador); pause_console(); continue; }
                else if (cmd == 'q') { char fname[128] = "savegame.dat"; if (save_game(&jogador, &mapa, fname)) printf("Jogo salvo em %s\n", fname); else printf("Falha ao salvar\n"); pause_console(); continue; }
                else if (cmd == 'e') { in_game = 0; continue; }
                ...
                if(moved){
                    if(map_check_encounter(&mapa, &jogador)){
                        ...
                        if(res == 0){
                            ...
                            int r = rand() % 100;
                            if(r < 40) {
                                Item it; it.tipo = ITEM_MUNI; strncpy(it.nome, "Municao", 31);
                        ...
                        if(mapa.grid[jogador.pos_y][jogador.pos_x] == TILE_ITEM){
                            Item it; it.tipo = ITEM_MEDKIT; strncpy(it.nome, "Medkit", 31); it.nome[31]='\0'; it.poder = 20; it.quantidade = 1;
                            inventory_add(&jogador.inventario, it);
                    ...
                    if(moved){
                        if(map_check_encounter(&mapa, &jogador)){
                            ...
                            if(res == 0){
                                ...
                                int r = rand() % 100;
                                if(r < 40){
                                    Item it; it.tipo = ITEM_MUNI; strncpy(it.nome, "Municao", 31); it.nome[31]='\0'; it.poder = 1; it.quantidade = 3; 
                                    inventory_add(&jogador.inventario, it);
                                    printf("O zumbi deixou municao!\n");
                                }
                                pause_console();
                            ...
                            if(mapa.grid[jogador.pos_y][jogador.pos_x] == TILE_ITEM){
                                Item it; it.tipo == ITEM_MEDKIT;
                                strncpy(it.nome, "Medkit", 31); it.nome[31]='\0'; it.poder = 20; it.quantidade = 1;
                                inventory_add(&jogador.inventario, it);
                                printf("Voce encontrou um item: Medkit!\n");
                                mapa.grid[jogador.pos_y][jogador.pos_x] = TILE_EMPTY;
                                pause_console();
